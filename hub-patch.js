...
/* ==== REEL GUARD v2 — 모바일=정적 콜라주(구멍 원천 차단) / 데스크톱=대칭 복구 ==== */
(function(){
  'use strict';
  function isMobile(){
  ...
  function uniqFrames(reel){
    var seen = {}, uniq = [];
    Array.prototype.forEach.call(reel.children, function(f){
      var k = keyOf(f);
      if (seen[k]) return;
  ...
  function injectMobileCSS(){
    if (document.getElementById('reel-mobile-css')) return;
    var st = document.createElement('style');
    st.id = 'reel-mobile-css';
    st.textContent =
      ...
      'header{position:sticky; top:0; z-index:40; padding-top:max(14px, env(safe-area-inset-top)) !important;}' +
      '.credits{margin-top:18px;}' +
      '.track{height:auto !important; overflow:visible !important; padding:4px 10px;}' +
      '.reel{position:static !important; height:auto !important; width:100% !important;' +
      ' animation:none !important; flex-wrap:wrap !important; justify-content:center !important;' +
      ' gap:12px !important; will-change:auto !important;}' +
      '.frame{height:auto !important;}' +
      '.frame .img{height:132px !important;}' +
      '.frame.land .img{width:170px !important;}' +
      '.frame.port .img{width:96px !important;}' +
      '}';
    document.head.appendChild(st);
  }
  function normalizeReel(reel){
    var uniq = uniqFrames(reel);
    if (!uniq.length) return;
    if (isMobile()){
      /* 모바일: 사진 1장씩만, 줄바꿈 콜라주 — 애니메이션 없음 = 빈칸 불가능 */
      injectMobileCSS();
      var frag = document.createDocumentFragment();
      uniq.forEach(function(f){
        var im = f.querySelector('img');
        if (!im || !im.getAttribute('src')) return;   /* 빈 필름(장식)은 모바일에서 제외 */
        frag.appendChild(f);
      });
      reel.innerHTML = '';
      reel.appendChild(frag);
      reel.setAttribute('data-guard', 'static');
      return;
    }
    /* 데스크톱: 두 절반이 항상 동일하도록 짝수 배 복제 (이음새 없는 루프 보장) */
    ...
    for (var r = 0; r < k2; r++)
      uniq.forEach(function(f){ frag2.appendChild(r === 0 ? f : f.cloneNode(true)); });
    reel.innerHTML = '';
  ...
  if (document.readyState === 'complete') setTimeout(runGuard, 300);
  window.addEventListener('load', function(){ setTimeout(runGuard, 300); });
  setTimeout(runGuard, 1200);
  setTimeout(runGuard, 3000);
  var rT; window.addEventListener('resize', function(){ clearTimeout(rT); rT = setTimeout(runGuard, 300); });
  window.addEventListener('orientationchange', function(){ setTimeout(runGuard, 400); });
})();
