(function(){
  'use strict';
  var STORE='fcos_personal_launch_v2',COOKIE='fcos_personal_launch_v1',deferred=null;
  ...
  function readCookie(){
    try{
      var hit=document.cookie.split(';').map(function(v){return v.trim();}).find(function(v){return v.indexOf(COOKIE+'=')===0;});
  ...
  function saveCookie(v){
    if(!valid(v))return;
    try{
      var raw=encodeURIComponent(JSON.stringify({u:v.u,n:v.n,k:v.k,savedAt:Date.now()}));
      document.cookie=COOKIE+'='+raw+'; Max-Age=31536000; Path=/callback/; SameSite=Lax'+(location.protocol==='https:'?'; Secure':'');
    }catch(e){}
  }
  function save(v){
    if(!valid(v))return;
    try{localStorage.setItem(STORE,JSON.stringify(Object.assign({},v,{savedAt:Date.now()})));localStorage.setItem('fcos_personal_launch',personalUrl(v));}catch(e){}
    saveCookie(v);
  }
  function clear(){
    try{localStorage.removeItem(STORE);localStorage.removeItem('fcos_personal_launch');}catch(e){}
    try{document.cookie=COOKIE+'=; Max-Age=0; Path=/callback/; SameSite=Lax'+(location.protocol==='https:'?'; Secure':'');}catch(e){}
  }
  ...
  var source='',identity=fromQuery();
  if(identity){source='query';save(identity);}
  else{
    identity=readCookie();source=valid(identity)?'cookie':'';
    if(!valid(identity)){identity=read();source=valid(identity)?'local':'';}
    ...
    if(valid(identity)){
      save(identity);
      var restored=new URL(personalUrl(identity));
      history.replaceState(null,'',restored.pathname+restored.search);
    }
  }
  /* iOS 홈 화면 웹앱은 Safari의 localStorage를 복사하지 않지만 쿠키는 복사한다.
     script.js보다 먼저 쿠키에서 개인 URL을 복원해 동일한 uid로 초기화한다. */
  window.__fcosLaunchRecovered=source;
  ...
  function refresh(v){if(!valid(v))return;identity={u:v.u,n:v.n,k:v.k};save(identity);var b=document.getElementById('personalInstallBtn');if(b)b.remove();draw();}
  window.FcosPersonalInstall={read:read,url:function(){return valid(identity)?personalUrl(identity):'';},guide:guide,copy:copy,refresh:refresh,clear:clear};
