...
const CONFIG=window.B2B_CONFIG||{};
const ZONES=CONFIG.zones||DEFAULT_ZONES;
const STORE_KEY=CONFIG.storeKey||'presence_b2b_guwol_v1';
...
function initMap(){
  ...
  map=L.map('map',{zoomControl:false,minZoom:15,maxZoom:20}).setView(CONFIG.center||[37.44545,126.70172],CONFIG.zoom||18);
  L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:20,attribution:'&copy; OpenStreetMap'}).addTo(map);
  L.control.zoom({position:'bottomright'}).addTo(map);
  L.rectangle(CONFIG.bounds||[[37.44312,126.70025],[37.44825,126.70385]],{color:'#58d39b',weight:2,fillColor:'#58d39b',fillOpacity:.045,dashArray:'7 8'}).addTo(map);
...
$('#reset').onclick=()=>{if(!confirm(`${CONFIG.locationName||'구월동'} 방문 결과와 메모를 모두 초기화할까요?`))return;state.zones={};save();renderList();refreshMarkers()};
